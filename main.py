...
class Config:
    ...
    init_opa: float = 0.07     #0.1!!!!!!!!!!
    # Initial scale of GS
    init_scale: float = 0.5    #1.0!!!!!!!!!
...
class SLAM():
    ...
    #===================================================================================#
        ...
        self.derdnet_cfg={
            'sub_radius_h': self.config.get('derdnet_sub_radius_h', 3),
    ...
    def load_derdnet_model(self):
        ...
        self.derdnet_model = PixelwiseConvGRU(
            sub_frame_radius_h=sub_r_h,
            sub_frame_radius_w=sub_r_w,
    ...
    def build_dsi_vectorized(self, events: torch.Tensor, 
        ...
        dsi = torch.zeros((H, W, num_depth_bins), device=self.device, dtype=torch.float32)

        num_events = events.shape[0]
        for start in range(0, num_events, event_batch_size):
            end = min(start + event_batch_size, num_events)
            batch_events = events[start:end]  # [B,4]
            B = batch_events.shape[0]

            x = batch_events[:, 0].long()
            y = batch_events[:, 1].long()
            t = batch_events[:, 2]
            p = batch_events[:, 3]
            ...
            p_flat = p.unsqueeze(1).expand_as(valid_mask)[valid_mask]

            dsi.index_put_((v_flat, u_flat, d_idx), p_flat, accumulate=True)

            del pt_cam, pt_world, pt_ref_cam, u, v
    ...
    def extract_sub_dsis_batch(self, dsi: torch.Tensor, pixel_coords: torch.Tensor):
        ...
        offsets = torch.stack(torch.meshgrid(
            ...
            torch.arange(-sub_r, sub_r+1, device=self.device),
            indexing='ij'
        ), dim=-1).reshape(-1, 2)  # [win_size, 2]

        y_center = pixel_coords[:, 0].unsqueeze(1)  # [M,1]
        x_center = pixel_coords[:, 1].unsqueeze(1)  # [M,1]
        y_idx = (y_center + offsets[:, 0].unsqueeze(0)).long()  # [M, win_size]
        x_idx = (x_center + offsets[:, 1].unsqueeze(0)).long()  # [M, win_size]

        y_idx = y_idx.clamp(0, H-1)
        x_idx = x_idx.clamp(0, W-1)

        dsi_flat = dsi.reshape(-1, D)  # [H*W, D]
        linear_idx = y_idx * W + x_idx  # [M, win_size]

        sub_vals = dsi_flat[linear_idx]  # [M, win_size, D]
        sub_vals = sub_vals.permute(0, 2, 1)  # [M, D, win_size]
        sub_vals = sub_vals.reshape(M, D, sub_size, sub_size)  # [M, D, sub_h, sub_w]

        return norm_pixel_coords, sub_vals
    #===========================================================================#
    def batch_infer_derdnet(self, norm_pixel_coords: torch.Tensor, sub_dsis: torch.Tensor) -> torch.Tensor:
        """
    ...
    def estimate_depth_map_derdnet(self, events: torch.Tensor, 
        ...
        pixel_coords = torch.stack([y_coords, x_coords], dim=1)  # [M,2]
        M = pixel_coords.shape[0]
        print(f"Selected {M} pixels with votes > {self.derdnet_cfg['min_votes']}")
...
#==============================================================================================#
    ...
    #=======================================================================================#
            ...
            if self.config["initialization"]["gaussian_init_sfm_mask"]==1: # using event map mask; 
                ...
                indice_h = pixel_uv_with_event[0][sampled_event_pixel_idx].to(self.device)
                indice_w = pixel_uv_with_event[1][sampled_event_pixel_idx].to(self.device)
                print(f"Sampled points: h range [{indice_h.min()}, {indice_h.max()}], w range [{indice_w.min()}, {indice_w.max()}]")
...
#============================================================================================#
    ...
    def get_poses(self, control_knot_poses, control_knot_ts, query_t, mode = 'cspline'):
        if mode == 'linear':
            ...
            pose_interp=cubic_bspline_interpolation(segment,tau_tensor)[0][0]
    ...
    def bundle_adjustment(self, BA_batch,  _traj_mode, _num_iters=100001, _seg_num=50, _opt_pose=True, _global_BA=False):
        ...
        #=================================================================================================================================#
            if _global_BA and self.depth_loss_weight>0:
                ...
                depth_gt_end=self.get_depth_at_time(t_ev_end,BA_batch)
                depth_start=render_pkg_start["depth"]
                depth_end=render_pkg_end["depth"]
                valid_start=(depth_gt_start>0).float()
                valid_end=(depth_gt_end>0).float()
                loss_depth_start=F.l1_loss(depth_start*valid_start,depth_gt_start*valid_start,reduction='sum')
                loss_depth_end=F.l1_loss(depth_end*valid_end,depth_gt_end*valid_end,reduction='sum')
                loss_depth=(loss_depth_start/(valid_start.sum()+1e-8)+loss_depth_end/(valid_end.sum()+1e-8))*0.5
                loss_depth_total=loss_depth_total+self.depth_loss_weight*loss_depth
        #=================================================================================================================================#
