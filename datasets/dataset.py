...
class ReplicaEventDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1,
        ...
        # self.events = np.load(os.path.join(basedir, 'event_threshold_0.1', 'gray_events_data.npy'), mmap_mode='r')
        #event_raw = np.load(os.path.join(basedir, 'event_threshold_0.1', 'gray_events_data.npy'))
        # print("Raw events first 10 rows:\n", self.events[:10])
        # print("Raw events dtype:", self.events.dtype)
        # print("Raw events shape:", self.events.shape)
        print("finished event loading!")
    ...
    def __getitem__(self, index):
        ...
        # selected_events = get_event_chunk(self.events, self.poses_ts[index-1], self.poses_ts[index])
        # print(f"Selected events shape: {selected_events.shape}")
        # print(f"x range: {selected_events[:,0].min()} - {selected_events[:,0].max()}")
        # print(f"y range: {selected_events[:,1].min()} - {selected_events[:,1].max()}")
        # print(f"t range: {selected_events[:,2].min()} - {selected_events[:,2].max()}")
        # print(f"p range: {selected_events[:,3].min()} - {selected_events[:,3].max()}")
        return ret
